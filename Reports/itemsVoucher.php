...
{
    // 50 es la altura fija que quieres darle a la imagen
    $this->Image($anulado, 5, 100, 200, 50, $ext_anulado);
}
...
// Client address
function addClientAdresse( $cliente, $domicilio ,$dni, $email, $telefono )
{
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 40;
...
{
    global $colonnes; // Nota: 'colonnes' se sobrescribe con $tab justo antes del bucle.
    ...
    $colX = $r1;
    $colonnes = $tab; // Aquí se asigna $tab a la variable global $colonnes

    // ¡Aquí está el cambio clave!
    // ANTES: while ( list( $lib, $pos ) = each ($tab) )
    // AHORA:
...
function addLineFormat( $tab )
{
    ...
    // Se cambió 'while ( list( $lib, $pos ) = each ($colonnes) )' a 'foreach ($colonnes as $lib => $pos)'
...
function lineVert( $tab )
{
    ...
    // La función 'reset()' sigue siendo válida, pero 'each()' se cambia.
    // 'reset($colonnes)' aún puede ser útil si el puntero interno del array
    // necesita ser reiniciado antes de la iteración.
    reset( $colonnes );
    $maxSize=0;
...
function addLine( $ligne, $tab )
{
    ...
    // La función 'reset()' aún es válida si se necesita reiniciar el puntero interno del array.
    reset( $colonnes );
    
    // ¡Aquí está el cambio clave! Reemplazamos 'while (list($lib, $pos) = each($colonnes))'
    // por un bucle 'foreach'.
    foreach ($colonnes as $lib => $pos)
    {
        $longCell    = $pos - 2;
        $texte       = $tab[ $lib ];
        ...
        $this->SetXY( $ordonnee, $ligne - 1 ); // Es buena práctica añadir espacios después de los operadores
        $this->MultiCell( $longCell, 4, $texte, 0, $formText );
        
        if ( $maxSize < $this->GetY() ) { // Aseguramos que la condición sea clara
            $maxSize = $this->GetY();
        }
        $ordonnee += $pos;
    }
